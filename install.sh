...
echo -e "${C}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${C}║        Ubuntu 24.04 LTS Setup (Ultimate Edition)       ║${NC}"
echo -e "${C}╚════════════════════════════════════════════════════════╝${NC}"
...
# ── 3. HOST DEPENDENCIES ──
echo -e "${Y}▸ Installing required host packages...${NC}"
apt-get update -y || true
apt-get install -y proot wget curl tmate sudo xz-utils 2>/dev/null || true
...
if [ ! -f "$ROOTFS_DIR/.setup_done" ]; then
    ...
    # FIX: Remove broken systemd symlink & copy host network settings
    ...
    echo -e "${Y}▸ Configuring APT (Disabling Sandboxes & Key Checks)...${NC}"

    proot -0 -r "$ROOTFS_DIR" -w / /bin/bash -c '
        export DEBIAN_FRONTEND=noninteractive

        # Disable apt user sandboxing (stops IPC pipe deadlocks inside proot)
...
EOF
        ...
        # Force trusted repositories (bypasses key errors and infinite download loops)
...
EOF
        ...
        apt-get update -y
        apt-get install -y --no-install-recommends curl wget vim nano htop tmux sudo openssh-client python3
        apt-get clean
    ...
    echo -e "${G}▸ Existing setup found. Skipping installation phase.${NC}"
fi

# ── 5. TMATE SESSION START ──
...
# Enforce a 10-second hard limit on tmate network setup
echo -e "${Y}▸ Fetching SSH/Web connection links (10s max wait)...${NC}"
timeout 10 tmate -S /tmp/tmate.sock wait tmate-ready 2>/dev/null || true

TMATE_SSH=$(tmate -S /tmp/tmate.sock display -p "#{tmate_ssh}" 2>/dev/null || echo "Connection timed out / Blocked by host firewall")
TMATE_WEB=$(tmate -S /tmp/tmate.sock display -p "#{tmate_web}" 2>/dev/null || echo "Connection timed out / Blocked by host firewall")
